...
static void MutateTxAddOutAddr(CMutableTransaction &tx,
    ...
    if (!addr.IsValid()) throw std::runtime_error("invalid TX output address");
...

            {
                ...
                if (prevOut.exists("amount")) {
                    coins->vout[nOut].nValue =
                        AmountFromValue(prevOut["amount"]);
                }
            }
...
static void OutputTxHash(const CTransaction &tx) {
    std::string strHexHash = tx.GetHash().GetHex(); // the hex-encoded
                                                    // transaction hash (aka the
                                                    // transaction id)
