...
#define MIN_TRANSACTION_BASE_SIZE                                              \
    (::GetSerializeSize(CTransaction(), SER_NETWORK,                           \
        ...
        for (size_t i = 0; i < vTxHashes.size(); i++) {
            ...
            if (idit != shorttxids.end()) {
                if (!have_txn[idit->second]) {
                    ...
                    // request it.
                    // This should be rare enough that the extra bandwidth
                    // doesn't matter,
                    // but eating a round-trip due to FillBlock failure would be
                    // annoying
    ...
    for (size_t i = 0; i < extra_txn.size(); i++) {
        ...
        if (idit != shorttxids.end()) {
            if (!have_txn[idit->second]) {
                ...
                // just
                // request it.
                // This should be rare enough that the extra bandwidth doesn't
                // matter,
                // but eating a round-trip due to FillBlock failure would be
                // annoying
                // Note that we dont want duplication between extra_txn and
                // mempool to
