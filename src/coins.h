...
/**
 * Pruned version of CTransaction: only retains metadata and unspent transaction
 * outputs
 *
 * Serialized format:
 * - VARINT(nVersion)
 * - VARINT(nCode)
 * - unspentness bitvector, for vout[2] and further; least significant byte
 * first
 ...
 * bitvector.
 *   - In case both bit 1 and bit 2 are unset, they encode N-1, as there must be
 * at
 ...
 *
 *    - version = 1
 *    - code = 4 (vout[1] is not spent, and 0 non-zero bytes of bitvector
 * follow)
 ...
 *                2 (1, +1 because both bit 1 and bit 2 are unset) non-zero
 * bitvector bytes follow)
 *  - unspentness bitvector: bits 2 (0x04) and 14 (0x4000) are set, so vout[2+2]
 * and vout[14+2] are unspent
 *  - vout[4]: 86ef97d5790061b01caab50f1b8e9c50a5057eb43c2d9563a4ee
 *             * 86ef97d579: compact amount representation for 234925952 (2.35
 * BTC)
 *             * 00: special txout type pay-to-pubkey-hash
...
struct CCoinsCacheEntry {
    ...
    enum Flags {
        ...
        FRESH = (1 << 1), // The parent view does not have this entry (or it is
                          // pruned).
...
private:
    ...
    size_t cachedCoinUsage; // Cached memory usage of the CCoins object before
                            // modification
...
public:
     ...
     * given
     * txid exists and creates a new one. This saves a database access in the
     * case where
     * the coins were to be wiped out by FromTx anyway.  This should not be
     * called with
     * the 2 historical coinbase duplicate pairs because the new coins are
     * marked fresh, and
     * in the event the duplicate coinbase was spent before a flush, the now
     * pruned coins
     * would not properly overwrite the first coinbase of the pair. Simultaneous
     * modifications
     * are not allowed.
     */
     ...
     * that are already in the chain.  These are the inputs that will age and
     * increase priority as
