        ...
        if (0 <= opcode && opcode <= OP_PUSHDATA4) {
            if (vch.size() <=
                ...
                if (fAttemptSighashDecode && !script.IsUnspendable()) {
                    ...
                    // data that looks like a signature within a scriptSig.
                    // this won't decode correctly formatted public keys in
                    // Pubkey or Multisig scripts due to
                    // the restrictions on the pubkey formats (see
                    // IsCompressedOrUncompressedPubKey) being incongruous with
                    // the
                    ...
                    if (CheckSignatureEncoding(vch, SCRIPT_VERIFY_STRICTENC,
                        ...
                        if (mapSigHashTypes.count(chSigHashType)) {
                            strSigHashDecode =
                                ...
                                "]";
                            vch.pop_back(); // remove the sighash type byte. it
                                            // will be replaced by the decode.
                        }
                    }
...
void TxToUniv(const CTransaction &tx, const uint256 &hashBlock,
    ...
    entry.pushKV("hex", EncodeHexTx(tx)); // the hex-encoded transaction. used
                                          // the name "hex" to be consistent
