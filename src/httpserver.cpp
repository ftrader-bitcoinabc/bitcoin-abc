...
static bool InitHTTPAllowList() {
    ...
    rpc_allow_subnets.push_back(
        CSubNet(localv6)); // always allow IPv6 localhost
...
static bool HTTPBindAddresses(struct evhttp *http) {
    ...
    if (!IsArgSet("-rpcallowip")) { // Default to loopback if not allowing
                                    // external IPs
...
void StopHTTPServer() {
    ...
    if (eventBase) {
        ...
        // responses), then break it
        // Before this was solved with event_base_loopexit, but that didn't work
        // as expected in
