...
public:
    ...
    bool GetCoins(const uint256 &txid, CCoins &coins) const {
        try {
            ...
            // would be
            // interpreted as 'entry not found' (as opposed to unable to read
            // data), and
            // could lead to invalid interpretation. Just exit immediately, as
            // we can't
...
std::string HelpMessage(HelpMessageMode mode) {
    ...
    // Do not translate _(...) -help-debug options, Many technical terms, and
    ...
    strUsage +=
        HelpMessageOpt("-rpcserialversion",
                       strprintf(_("Sets the serialization of raw transaction "
                                   "or block hex returned in non-verbose mode, "
    ...
    strUsage += HelpMessageOpt("-mempoolreplacement",
                               strprintf(_("Enable transaction replacement in "
                                           "the memory pool (default: %u)"),
...
void InitParameterInteraction() {
    ...
    if (IsArgSet("-proxy")) {
        ...
        // to listen locally, so don't rely on this happening through -listen
        // below.
...
bool AppInitParameterInteraction() {
    ...
    // BIP 125 replacement in the mempool
    ...
    while (!fLoaded) {
        ...
        do {
            try {
                ...
                // is a user who has pruned blocks
...
// ********************************************************* Step 8: load wallet
    ...
    if (chainparams.GetConsensus()
            ...
            .nTimeout != 0) {
        // Only advertise witness capabilities if they have a reasonable start
        // time.
        // This allows us to have the code merged without a defined softfork, by
        // setting its
        // end time to 0.
        // Note that setting NODE_WITNESS is never required: the only downside
        // from not
        // doing so is that after activation, no upgraded nodes will fetch from
        // you.
