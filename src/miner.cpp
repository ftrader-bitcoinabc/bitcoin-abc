...
BlockAssembler::BlockAssembler(const CChainParams &_chainparams)
    ...
    nBlockMaxWeight = std::max(
        (unsigned int)4000,
        std::min((unsigned int)(MAX_BLOCK_WEIGHT - 4000), nBlockMaxWeight));
    // Limit size to between 1K and MAX_BLOCK_SERIALIZED_SIZE-1K for sanity:
    nBlockMaxSize =
        std::max((unsigned int)1000,
                 std::min((unsigned int)(MAX_BLOCK_SERIALIZED_SIZE - 1000),
                          nBlockMaxSize));
...
BlockAssembler::CreateNewBlock(const CScript &scriptPubKeyIn,
    ...
    fIncludeWitness =
        IsWitnessEnabled(pindexPrev, chainparams.GetConsensus()) &&
        fMineWitnessTx;
    ...
    LogPrintf("CreateNewBlock(): total size: %u block weight: %u txs: %u fees: "
              "%ld sigops %d\n",
              nSerializeSize, GetBlockWeight(*pblock), nBlockTx, nFees,
              nBlockSigOpsCost);
...
bool BlockAssembler::TestPackageTransactions(
    ...
    uint64_t nPotentialBlockSize =
        nBlockSize; // only used with fNeedSizeAccounting
...
bool BlockAssembler::TestForBlock(CTxMemPool::txiter iter) {
    if (nBlockWeight + iter->GetTxWeight() >= nBlockMaxWeight) {
        ...
        // Once we're within 4000 weight of a full block, only look at 50 more
        // txs
    ...
    if (fNeedSizeAccounting) {
        if (nBlockSize + ::GetSerializeSize(iter->GetTx(), SER_NETWORK,
                                            PROTOCOL_VERSION) >=
            nBlockMaxSize) {
...
void BlockAssembler::addPriorityTxs() {
    ...
    CTxMemPool::txiter iter;
    while (!vecPriority.empty() && !blockFinished) { // add a tx from priority
                                                     // queue to fill the
                                                     // blockprioritysize
        ...
        if (!fIncludeWitness && iter->GetTx().HasWitness()) continue;
        ...
        if (TestForBlock(iter)) {
            ...
            // priority size
            // or have dropped below the AllowFreeThreshold, then we're done
            // adding priority txs
            ...
            // add transactions that depend on this one to the priority queue to
            // try again
