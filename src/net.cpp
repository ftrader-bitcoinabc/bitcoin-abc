...
CNode *CConnman::ConnectNode(CAddress addrConnect, const char *pszDest,
    ...
    if (pszDest ? ConnectSocketByName(addrConnect, hSocket, pszDest,
        ...
        if (pszDest && addrConnect.IsValid()) {
            ...
            // pszDest resolved to.
            // In that case, drop the connection that was just created, and
            // return the existing CNode instead.
            // Also store the name we used to connect in that CNode, so that
            // future FindNode() calls to that
...
void CConnman::Ban(const CSubNet &subNet, const BanReason &banReason,
    ...
    if (banReason == BanReasonManuallyAdded)
        DumpBanlist(); // store banlist to disk immediately if user requested
                       // ban
}
    ...
    while (!interruptNet) {
...

        {
            ...
            BOOST_FOREACH (CNode *pnode, vNodes) {
                ...
                //   drain the
                //   write buffer in this case before receiving more. This
                //   avoids
                //   needlessly queueing received data, if the remote peer is
                //   not themselves
                //   receiving data. This means properly utilizing TCP flow
                //   control signalling.
...
void CConnman::ThreadDNSAddressSeed() {
    ...
    //  less influence on the network topology, and reduces traffic to the
    //  seeds.
    if ((addrman.size() > 0) &&
    ...
    BOOST_FOREACH (const CDNSSeedData &seed, vSeeds) {
        if (HaveNameProxy()) {
            ...
            if (LookupHost(GetDNSHost(seed, &requiredServiceBits).c_str(), vIPs,
                ...
                BOOST_FOREACH (const CNetAddr &ip, vIPs) {
                    ...
                    addr.nTime = GetTime() - 3 * nOneDay -
                                 GetRand(4 * nOneDay); // use a random age
            ...
            // which results in
            // addrman assigning the same source to results from different
            // seeds.
            // This should switch to a hard-coded stable dummy IP for each seed
            // name, so that the
...
void CConnman::ThreadOpenConnections() {
    ...
    while (!interruptNet) {
        ...
        std::set<std::vector<unsigned char>> setConnected;
        {
            ...
            BOOST_FOREACH (CNode *pnode, vNodes) {
                if (!pnode->fInbound && !pnode->fAddnode) {
                    ...
                    // is to not use multiple of our limited outbound slots on a
                    // single netgroup
                    // but inbound and addnode peers do not use our outbound
                    // slots.  Inbound peers
                    // also have the added issue that they're attacker
                    // controlled and could be used
                    // to prevent us from connecting to particular hosts if we
                    // used them here.
        ...
        if (nOutbound >= nMaxOutbound) {
            int64_t nTime = GetTimeMicros(); // The current time right now (in
                                             // microseconds).
        ...
        while (!interruptNet) {
            ...
            // addresses fetched from addrman,
            // stop this loop, and let the outer loop run again (which sleeps,
            // adds seed nodes, recalculates
        ...
        for (const AddedNodeInfo &info : vInfo) {
            if (!info.fConnected) {
                if (!grant.TryAcquire()) {
                    ...
                    // not wait here since while we are waiting
...
void CConnman::SetNetworkActive(bool active) {
    ...
    if (!active) {
        ...
        BOOST_FOREACH (CNode *pnode, vNodes) { pnode->CloseSocketDisconnect(); }
    } else {
        fNetworkActive = true;
    }
...
void CConnman::Stop() {
    ...
    BOOST_FOREACH (CNode *pnode, vNodes) { DeleteNode(pnode); }
    BOOST_FOREACH (CNode *pnode, vNodesDisconnected) { DeleteNode(pnode); }
    vNodes.clear();
