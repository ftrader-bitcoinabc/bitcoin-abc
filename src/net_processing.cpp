...
struct CNodeState {
     ...
     * otherwise: whether this peer sends non-witnesses in
...
void FindNextBlocksToDownload(NodeId nodeid, unsigned int count,
    ...
    // than BLOCK_DOWNLOAD_WINDOW + 1 beyond the last
    ...
    while (pindexWalk->nHeight < nMaxHeight) {
        ...
        // the ones that
        // are not yet downloaded and not in flight to vBlocks. In the mean
        // time, update
        // pindexLastCommonBlock as long as all ancestors are already
        // downloaded, or if it's
        // already part of our chain (and therefore don't need it even if
        // pruned).
        BOOST_FOREACH (const CBlockIndex *pindex, vToFetch) {
            ...
            if (!State(nodeid)->fHaveWitness &&
                ...
                // We wouldn't download this block or its descendants from this
                // peer.
                return;
            }
...
void PeerLogicValidation::NewPoWValidBlock(
    ...
    connman->ForEachNode([this, &pcmpctblock, pindex, &msgMaker,
                          fWitnessEnabled, &hashBlock](CNode *pnode) {
...
bool static AlreadyHave(const CInv &inv) EXCLUSIVE_LOCKS_REQUIRED(cs_main) {
    switch (inv.type) {
        ...
        case MSG_WITNESS_TX: {
            ...
            // exclude
            // requesting or processing some txs which have already been
            // included in a block
            ...
            if (inv.type == MSG_BLOCK || inv.type == MSG_FILTERED_BLOCK ||
                ...
                if (mi != mapBlockIndex.end()) {
                    if (mi->second->nChainTx &&
                        ...
                        // we might be in the middle of connecting it (ie in the
                        // unlock of cs_main
                        // before ActivateBestChain but after AcceptBlock).
                        // In this case, we need to run ActivateBestChain prior
                    ...
                    if (chainActive.Contains(mi->second)) {
                        ...
                        // chain if they are valid, and no more than a month
                        // older (both in time, and in
                        // best equivalent proof of work) than the best header
                        // chain we know about.
                ...
                if (send && (mi->second->nStatus & BLOCK_HAVE_DATA)) {
                    ...
                    if (inv.type == MSG_BLOCK)
                        ...
                        if (sendMerkleBlock) {
                            ...
                            // see
                            // This avoids hurting performance by pointlessly
                            // requiring a round-trip
                            // Note that there is currently no way for a node to
                            // request any single transactions we didn't send
                            // here -
                            // they must either disconnect and retry or request
                            // the full block.
                            // Thus, the protocol spec specified allows for us
                            // to provide duplicate txn here,
                        ...
                        // guaranteed
                        // they won't have a useful mempool to match against a
                        // compact block,
                        // and we don't feel like constructing the object for
                        // them, so
                        // instead we respond with the full, non-compact block.
                        bool fPeerWantsWitness =
                            State(pfrom->GetId())->fWantsCmpctWitness;
                        int nSendFlags = fPeerWantsWitness
                                             ? 0
                                             : SERIALIZE_TRANSACTION_NO_WITNESS;
                    ...
                    if (inv.hash == pfrom->hashContinue) {
                        ...
                        // redundant,
                        // and we want it right after the last block so they
                        // don't
                ...
                if (mi != mapRelay.end()) {
                    ...
                    // mempool when
                    // that TX couldn't have been INVed in reply to a MEMPOOL
                    // request.
                    if (txinfo.tx &&
...
bool static ProcessMessage(CNode *pfrom, const std::string &strCommand,
    ...
    if (strCommand == NetMsgType::VERACK) {
        ...
        if (nCMPCTBLOCKVersion == 1 ||
            ...
            // blocks we send (fWantsCmpctWitness)
        ...
        if (it->second->nHeight < chainActive.Height() - MAX_BLOCKTXN_DEPTH) {
            ...
            CInv inv;
            inv.type = State(pfrom->GetId())->fWantsCmpctWitness
                           ? MSG_WITNESS_BLOCK
                           : MSG_BLOCK;
        ...
        if (!AlreadyHave(inv) &&
            ...
            LogPrint("mempool", "AcceptToMemoryPool: peer=%d: accepted %s "
                                "(poolsz %u txn, %u kB)\n",
                     pfrom->id, tx.GetHash().ToString(), mempool.size(),
            ...
            if (!tx.HasWitness() && !state.CorruptionPossible()) {
                ...
                if (RecursiveDynamicUsage(*ptx) < 100000) {
                    AddToCompactExtraTransactions(ptx);
                       ...
                       RecursiveDynamicUsage(*ptx) < 100000) {
                AddToCompactExtraTransactions(ptx);
            }
            ...
            if (fCanDirectFetch && pindexLast->IsValid(BLOCK_VALID_TREE) &&
                ...
                while (pindexWalk && !chainActive.Contains(pindexWalk) &&
                    ...
                    if (!(pindexWalk->nStatus & BLOCK_HAVE_DATA) &&
                        ...
                        (!IsWitnessEnabled(pindexWalk->pprev,
                                           chainparams.GetConsensus()) ||
                         State(pfrom->GetId())->fHaveWitness)) {
        ...
        //
        {
            ...
            if (!fRevertToInv && !vHeaders.empty()) {
                if (vHeaders.size() == 1 && state.fPreferHeaderAndIDs) {
                    ...
                    int nSendFlags = state.fWantsCmpctWitness
                                         ? 0
                                         : SERIALIZE_TRANSACTION_NO_WITNESS;
                    ...
                    bool fGotBlockFromCache = false;
                    {
                        ...
                        if (most_recent_block_hash ==
                            ...
                            if (state.fWantsCmpctWitness)
                                connman.PushMessage(
                                    pto, msgMaker.Make(
                                             nSendFlags, NetMsgType::CMPCTBLOCK,
                                             *most_recent_compact_block));
                            else {
                                CBlockHeaderAndShortTxIDs cmpctblock(
                                    *most_recent_block,
                    ...
                    if (!fGotBlockFromCache) {
                        ...
                        assert(ret);
                        CBlockHeaderAndShortTxIDs cmpctblock(
                            block, state.fWantsCmpctWitness);
                        connman.PushMessage(
                            pto,
