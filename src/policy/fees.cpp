...
double TxConfirmStats::EstimateMedianVal(int confTarget, double sufficientTxVal,
    ...
    // successively
    // smaller buckets until we reach failure.  Otherwise, we are looking for
    // the highest
    ...
    for (int bucket = startbucket; bucket >= 0 && bucket <= maxbucketindex;
        ...
        // (Only count the confirmed data points, so that each confirmation
        // count
...
void CBlockPolicyEstimator::processTransaction(const CTxMemPoolEntry &entry,
    ...
    if (txHeight != nBestSeenHeight) {
        ...
        // 1-block reorgs.
        // Ignore txs if BlockPolicyEstimator is not in sync with
        // chainActive.Tip().
