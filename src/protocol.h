...
enum ServiceFlags : uint64_t {
    ...
    // It is currently
    // set by all Bitcoin Core nodes, and is unset by SPV clients or other peers
    // that just want
    // network services but don't provide them.
    NODE_NETWORK = (1 << 0),
    // NODE_GETUTXO means the node is capable of responding to the getutxo
    // protocol request.
    // Bitcoin Core does not support this but a patch set called Bitcoin XT
    // does.
    ...
    NODE_BLOOM = (1 << 2),
    // NODE_WITNESS indicates that a node can be asked for blocks and
