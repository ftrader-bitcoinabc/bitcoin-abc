...
void CoinControlDialog::viewItemChanged(QTreeWidgetItem *item, int column) {
                                                  ...
                                                  // parent node in tree mode)
    {
        ...
        else if (item->isDisabled()) // locked (this happens if "check all"
                                     // through parent node)
        ...
        if (ui->treeWidget->isEnabled()) // do not update on every click for
                                         // (un)select all
            CoinControlDialog::updateLabels(model, this);
    }
...
void CoinControlDialog::updateLabels(WalletModel *model, QDialog *dialog) {
    ...
    if (nQuantity > 0) {
        ...
        if (fWitness) {
            // there is some fudging in these numbers related to the actual
            // virtual transaction size calculation that will keep this estimate
            // from being exact.
            // usually, the result will be an overestimate within a couple of
            // satoshis so that the confirmation dialog ends up displaying a
            // slightly smaller fee.
            // also, the witness stack size value value is a variable sized
            // integer. usually, the number of stack items will be well under
            // the single byte var int limit.
        ...
        dPriority = dPriorityInputs /
                    (nBytes - nBytesInputs +
                     (nQuantityUncompressed * 29)); // 29 = 180 - 151
                                                    // (uncompressed public keys
                                                    // are over the limit. max
                                                    // 151 bytes of the input
                                                    // are ignored for priority)
            ...
            if (nChange > 0 && nChange < MIN_CHANGE) {
                ...
                if (txout.IsDust(dustRelayFee)) {
                    if (CoinControlDialog::
                            fSubtractFeeFromAmount) // dust-change will be
                                                    // raised until no dust
    ...
    l4->setText(
        BitcoinUnits::formatWithUnit(nDisplayUnit, nAfterFee)); // After Fee
    ...
    BOOST_FOREACH (const PAIRTYPE(QString, std::vector<COutput>) & coins,
        ...
        BOOST_FOREACH (const COutput &out, coins.second) {
            ...
            itemOutput->setData(
                ...
                QVariant(
                    (qlonglong)out.tx->tx->vout[out.i]
