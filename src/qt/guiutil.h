...
/** Get save filename, mimics QFileDialog::getSaveFileName, except that it
   ...
   (*.csv)"
      @param[out] selectedSuffixOut  Pointer to return the suffix (file type)
   that was selected (or 0).
                  Can be useful when choosing the save file format based on
   suffix.
     */
...
/** Qt event filter that intercepts ToolTipChange events, and replaces the
   ...
   messages.
      Tooltips longer than the provided size threshold (in characters) are
   wrapped.
     */
...
/**
     * Makes a QTableView last column feel as if it was being resized from its
 * left border.
     * Also makes sure the column widths are never larger than the table's
 * viewport.
 ...
 * strech mode, columns aren't resizable
