...
void PaymentServer::ipcParseCommandLine(int argc, char *argv[]) {
    for (int i = 1; i < argc; i++) {
        ...
        // detect the
        // network as that would require fetching and parsing the payment
        // request.
        // That means clicking such an URI which contains a testnet payment
        // request
...
void PaymentServer::uiReady() {
    ...
    saveURIs = false;
    Q_FOREACH (const QString &s, savedPaymentRequests) { handleURIOrFile(s); }
...
bool PaymentServer::processPaymentRequest(const PaymentRequestPlus &request,
    ...
    Q_FOREACH (const PAIRTYPE(CScript, CAmount) & sendingTo, sendingTos) {
        ...
        // but CAmount is defined as int64_t. Because of that we need to verify
        // that amounts are in a valid range
