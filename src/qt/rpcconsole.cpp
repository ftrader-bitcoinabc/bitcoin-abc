                        ...
                        if (state == STATE_COMMAND_EXECUTED_INNER) {
                            ...
                            if (curarg.size() && fExecute) {
                                ...
                                if (lastResult.isArray()) {
                                    ...
                                    throw std::runtime_error(
                                        "Invalid result query"); // no array or
                                                                 // object:
                                                                 // abort
                    ...
                    case '\n':
                        ...
                        if ((ch == ')' || ch == '\n') && stack.size() > 0) {
                            if (fExecute) {
                                ...
                                // method-dependent way,
                                // and pass it along with the method name to the
                                // dispatcher.
                                JSONRPCRequest req;
        ...
        try // Nice formatting for standard-format error
        {
            ...
            Q_EMIT reply(RPCConsole::CMD_ERROR,
                         QString::fromStdString(message) + " (code " +
                             QString::number(code) + ")");
        } catch (const std::runtime_error &) // raised when converting to
                                             // invalid type, i.e. missing code
                                             // or message
    ...
    if (event->type() == QEvent::KeyPress) // Special key handling
    {
        ...
        switch (key) {
            ...
            default:
                ...
                // redirects key there
                // Exclude most combinations and keys that emit no text, except
                // paste shortcuts
