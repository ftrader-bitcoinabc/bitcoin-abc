...
void RPCNestedTests::rpcNestedTests() {
    ...
    RPCConsole::RPCExecuteCommandLine(
        result, "getblock( getblock( getblock(getbestblockhash())[hash] "
                ")[hash], true)"); // 4 level nesting with whitespace, filtering
                                   // path and boolean parameter
    ...
    RPCConsole::RPCExecuteCommandLine(
        result,
        "getblockchaininfo "); // whitespace at the end will be tolerated
    QVERIFY(result.substr(0, 1) == "{");
    ...
    (RPCConsole::RPCExecuteCommandLine(
        result2, "createrawtransaction( [],  {} , 0   )")); // whitespace
                                                            // between
                                                            // parametres is
                                                            // allowed
    QVERIFY(result == result2);
