...
public:
    ...
    void updateWallet(const uint256 &hash, int status, bool showTransaction) {
        ...
        if (status == CT_UPDATED) {
            if (showTransaction && !inModel)
                status =
                    CT_NEW; /* Not in model, but want to show, treat as new */
            if (!showTransaction && inModel)
                status = CT_DELETED; /* In model, but want to hide, treat as
                                        deleted */
        }
        ...
        switch (status) {
            ...
            case CT_UPDATED:
                ...
                // take care of this, and is only computed for
    ...
    TransactionRecord *index(int idx) {
        if (idx >= 0 && idx < cachedWallet.size()) {
            ...
            //  update the status of this transaction from the wallet.
            //  Otherwise,
