        ...
        Q_FOREACH (const SendCoinsRecipient &rcp, transaction.getRecipients()) {
            if (rcp.paymentRequest.IsInitialized()) {
                ...
                newTx->vOrderForm.push_back(make_pair(key, value));
                            ...
                            .isEmpty()) // Message from normal bitcoin:URI
...
bool WalletModel::changePassphrase(const SecureString &oldPass,
    ...
    {
        LOCK(wallet->cs_wallet);
        wallet->Lock(); // Make sure wallet is locked before attempting pass
                        // change
...
void WalletModel::loadReceiveRequests(
    ...
    BOOST_FOREACH (const PAIRTYPE(CTxDestination, CAddressBookData) & item,
        ...
        BOOST_FOREACH (const PAIRTYPE(std::string, std::string) & item2,
