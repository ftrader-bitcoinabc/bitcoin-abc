...
struct CCoin {
    uint32_t nTxVer; // Don't call this nVersion, that name has a special
                     // meaning inside IMPLEMENT_SERIALIZE
    uint32_t nHeight;
    CTxOut out;
...
static bool rest_getutxos(HTTPRequest *req, const std::string &strURIPart) {
        ...
        case RF_BINARY: {
            try {
                ...
                if (strRequestMutable.size() > 0) {
                    if (fInputParsed) // don't allow sending input over URI and
                                      // HTTP RAW DATA
                        return RESTERR(req, HTTP_BAD_REQUEST,
    ...
    bitmap.resize((vOutPoints.size() + 7) / 8);
    {
        ...
        if (fCheckMemPool)
            view.SetBackend(viewMempool); // switch cache backend to db+mempool
                                          // in case user likes to query mempool
        ...
        for (size_t i = 0; i < vOutPoints.size(); i++) {
            ...
            if (view.GetCoins(hash, coins)) {
                ...
                if (coins.IsAvailable(vOutPoints[i].n)) {
                    ...
                    // if it's off the end of the array, or if
                    // n is valid but points to an already spent output
                    // (IsNull).
                    CCoin coin;
