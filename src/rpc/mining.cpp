...
UniValue getblocktemplate(const JSONRPCRequest &request) {
    if (request.fHelp || request.params.size() > 1)
        throw runtime_error(
            ...
            "zero\n"
            "         \"weight\" : n,                (numeric) total "
            "transaction weight, as counted for purposes of block limits\n"
    ...
    const struct BIP9DeploymentInfo &segwit_info =
    ...
    if (nMaxVersionPreVB >= 2) {
        ...
        // get here
        // Because BIP 34 changed how the generation transaction is serialized,
        // we can only use version/force back to v2 blocks
        ...
        // Note that this can probably also be removed entirely after the first
        // BIP9 non-force deployment (ie, probably segwit) gets activated
    ...
    if (!pblocktemplate->vchCoinbaseCommitment.empty() && fSupportsSegwit) {
        result.push_back(
            Pair("default_witness_commitment",
...
UniValue estimatefee(const JSONRPCRequest &request) {
    if (request.fHelp || request.params.size() != 1)
        throw runtime_error(
            ...
            "confirmation within nblocks blocks. Uses virtual transaction size "
            "of transaction\n"
