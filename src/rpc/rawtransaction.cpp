...
void TxToJSON(const CTransaction &tx, const uint256 hashBlock,
    ...
    for (unsigned int i = 0; i < tx.vin.size(); i++) {
        ...
        if (tx.HasWitness()) {
            ...
            for (unsigned int j = 0; j < tx.vin[i].scriptWitness.stack.size();
                 j++) {
                std::vector<unsigned char> item =
                    tx.vin[i].scriptWitness.stack[j];
...
UniValue getrawtransaction(const JSONRPCRequest &request) {
    if (request.fHelp || request.params.size() < 1 || request.params.size() > 2)
        throw runtime_error(
            ...
            "size\n"
            "  \"vsize\" : n,            (numeric) The virtual transaction "
            "size (differs from size for witness transactions)\n"
    ...
    for (unsigned int idx = 0; idx < txids.size(); idx++) {
        ...
        if (txid.get_str().length() != 64 || !IsHex(txid.get_str()))
            throw JSONRPCError(RPC_INVALID_PARAMETER,
                               string("Invalid txid ") + txid.get_str());
        uint256 hash(uint256S(txid.get_str()));
        if (setTxids.count(hash))
            throw JSONRPCError(RPC_INVALID_PARAMETER,
                               string("Invalid parameter, duplicated txid: ") +
                                   txid.get_str());
...
UniValue sendrawtransaction(const JSONRPCRequest &request) {
    ...
    if (!fHaveMempool && !fHaveChain) {
        ...
        if (!AcceptToMemoryPool(mempool, state, std::move(tx), fLimitFree,
                                &fMissingInputs, NULL, false, nMaxRawTxFee)) {
            if (state.IsInvalid()) {
