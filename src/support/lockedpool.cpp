...
void *Win32LockedPageAllocator::AllocateLocked(size_t len,
    ...
    if (addr) {
        ...
        // Note
        // that it does not provide this as a guarantee, but, in practice,
        // memory
...
bool LockedPool::new_arena(size_t size, size_t align) {
    ...
    // in this case no memory can be locked at all so we'll skip past this
    // logic.
    if (arenas.empty()) {
    ...
    if (locked) {
        ...
        if (!lf_cb()) { // If the callback returns false, free the memory and
                        // fail, otherwise consider the user warned and proceed.
