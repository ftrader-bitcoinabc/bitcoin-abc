...
BOOST_AUTO_TEST_CASE(coins_cache_simulation_test) {
    ...
    for (unsigned int i = 0; i < NUM_SIMULATION_ITERATIONS; i++) {
        ...
        {
            uint256 txid =
                txids[insecure_rand() % txids.size()]; // txid we're going to
                                                       // modify in this
                                                       // iteration.
        ...
        if (insecure_rand() % 1000 == 1 || i == NUM_SIMULATION_ITERATIONS - 1) {
            for (std::map<uint256, CCoins>::iterator it = result.begin();
                 it != result.end(); it++) {
...
BOOST_AUTO_TEST_CASE(updatecoins_simulation_test) {
    ...
    for (unsigned int i = 0; i < NUM_SIMULATION_ITERATIONS; i++) {
        ...
        if (randiter % 20 < 19) {
            ...
            tx.vout.resize(1);
            tx.vout[0].nValue =
                i; // Keep txs unique unless intended to duplicate
            ...
            // 17/20 times reconnect previous or add a regular tx
                ...
                // will work properly
                // if that ever happens and not resurrect the previously
            ...
            alltxs.insert(std::make_pair(tx.GetHash(),
        ...
        // 1/20 times undo a previous transaction
            ...
            if (!tx.IsCoinBase()) utxoset.insert(tx.vin[0].prevout.hash);
        }
...
void CheckAccessCoins(CAmount base_value, CAmount cache_value,
