...
BOOST_AUTO_TEST_CASE(iterator_ordering) {
    ...
    for (int c = 0; c < 2; ++c) {
        ...
        for (int x = seek_start; x < 256; ++x) {
            ...
            if (!it->Valid()) // Avoid spurious errors about invalid iterator's
                              // key and value in case of failure
                break;
    ...
    for (int c = 0; c < 2; ++c) {
        ...
        for (int x = seek_start; x < 10; ++x) {
            for (int y = 0; y < 10; y++) {
                ...
                uint32_t value;
                BOOST_CHECK(it->Valid());
                if (!it->Valid()) // Avoid spurious errors about invalid
                                  // iterator's key and value in case of failure
                    break;
                BOOST_CHECK(it->GetKey(key));
                BOOST_CHECK(it->GetValue(value));
