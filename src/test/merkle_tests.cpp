...
BOOST_AUTO_TEST_CASE(merkle_test) {
    for (int i = 0; i < 32; i++) {
        ...
        for (int mutate = 0; mutate <= 3; mutate++) {
            ...
            if (duplicate1 >= ntx)
                break; // Duplication of the entire tree results in a different
                       // root (it adds a level).
            int ntx1 = ntx + duplicate1; // The resulting number of transactions
                                         // after the first duplication.
            int duplicate2 = mutate >= 2
                                 ? 1 << ctz(ntx1)
            ...
            if (mutate == 0) {
                for (int loop = 0; loop < std::min(ntx, 16); loop++) {
                    ...
                    BOOST_CHECK(
                        ComputeMerkleRootFromBranch(block.vtx[mtx]->GetHash(),
