        ...
        if (test[0].isArray()) {
            ...
            for (unsigned int i = 0; i < tx.vin.size(); i++) {
                ...
                BOOST_CHECK_MESSAGE(
                    VerifyScript(
                        tx.vin[i].scriptSig,
                        mapprevOutScriptPubKeys[tx.vin[i].prevout], witness,
                        verify_flags,
...
SetupDummyInputs(CBasicKeyStore &keystoreRet, CCoinsViewCache &coinsRet) {
    ...
    coinsRet.ModifyCoins(dummyTransactions[0].GetHash())
    ...
    coinsRet.ModifyCoins(dummyTransactions[1].GetHash())
...
void CreateCreditAndSpend(const CKeyStore &keystore, const CScript &outscript,
    ...
    assert(input.vin[0].scriptWitness.stack ==
           inputm.vin[0].scriptWitness.stack);
}
