...
double CTxMemPoolEntry::GetPriority(unsigned int currentHeight) const {
    ...
    if (dResult < 0) // This should only happen if it was called with a height
                     // below entry height
        dResult = 0;
    return dResult;
}
...
bool CTxMemPool::addUnchecked(const uint256 &hash, const CTxMemPoolEntry &entry,
    ...
    // An exception to that is if a transaction enters that used to be in a
    // block.
...
void CTxMemPool::check(const CCoinsViewCache *pcoins) const {
    ...
    for (indexed_transaction_set::const_iterator it = mapTx.begin();
        ...
        for (; iter != mapNextTx.end() &&
               iter->first->hash == it->GetTx().GetHash();
             ++iter) {
...
bool CCoinsViewMemPool::GetCoins(const uint256 &txid, CCoins &coins) const {
    ...
    // it contains full)
    // transactions. First checking the underlying cache risks returning a
    // pruned entry instead.
