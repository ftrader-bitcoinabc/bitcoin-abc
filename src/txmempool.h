 ...
 * nTxSize/
 * nFee+feeDelta. (This can potentially happen during a reorg, where we limit
 * the
...
private:
    ...
    CAmount inChainInputValue; //!< Sum of all txin values that are already in
                               //!blockchain
...
public:
     ...
     *  UpdateTransactionsFromBlock() will find child transactions and update
     * the
     *  descendant state for each transaction in hashesToUpdate (excluding any
     *  child transactions present in hashesToUpdate, which are already
     * accounted
     ...
     * sizelimit.
      *  pvNoSpendsRemaining, if set, will be populated with the list of
     * transactions
      *  which are not in mempool which no longer have any spends in this
     * mempool.
      */
...
private:
     ...
     *  state).
     *
     *  cachedDescendants will be updated with the descendants of the
     * transaction
...
struct TxCoinAgePriorityCompare {
    bool operator()(const TxCoinAgePriority &a, const TxCoinAgePriority &b) {
        if (a.first == b.first)
            return CompareTxMemPoolEntryByScore()(
                *(b.second),
                *(a.second)); // Reverse order to make sort less than
