...
public:
    CInit() {
        ...
        // loadable modules and engines.
        // We don't use them so we don't require the config. However some of our
        // libs may call functions
        // which attempt to load the config file, possibly resulting in an
        // exit() or crash if it is missing
...
void ReadConfigFile(const std::string &confPath) {
    ...
    if (!streamConfig.good()) return; // No bitcoin.conf file is OK
