        ...
        BOOST_FOREACH (const CTxIn &txin, tx.vin) {
            ...
            if (itConflicting != pool.mapNextTx.end()) {
                ...
                if (!setConflicts.count(ptxConflicting->GetHash())) {
                    ...
                    // non-replaceable transactions. All inputs rather than just
                    // one
                    ...
                    // unconfirmed ancestors anyway; doing otherwise is
                    // hopelessly
                    // insecure.
                    bool fReplacementOptOut = true;
                    if (fEnableReplacement) {
                        BOOST_FOREACH (const CTxIn &_txin,
                                       ptxConflicting->vin) {
                            if (_txin.nSequence <
...

    {
        ...
        BOOST_FOREACH (CTxMemPool::txiter ancestorIt, setAncestors) {
            ...
            if (setConflicts.count(hashAncestor)) {
                return state.DoS(
                    10, false, REJECT_INVALID, "bad-txns-spends-conflicting-tx",
                    false, strprintf("%s spends conflicting transaction %s",
        ...
        if (fReplacementTransaction) {
            ...
            BOOST_FOREACH (const uint256 &hashConflicting, setConflicts) {
                ...
                if (newFeeRate <= oldFeeRate) {
                    return state.DoS(
                        0, false, REJECT_INSUFFICIENTFEE, "insufficient fee",
                        false, strprintf("rejecting replacement %s; new "
                                         "feerate %s <= old feerate %s",
            ...
            for (unsigned int j = 0; j < tx.vin.size(); j++) {
                ...
                if (!setConflictsParents.count(tx.vin[j].prevout.hash)) {
                    ...
                    if (pool.mapTx.find(tx.vin[j].prevout.hash) !=
                        pool.mapTx.end())
                        return state.DoS(
                            0, false, REJECT_NONSTANDARD,
            ...
            if (nDeltaFees < ::incrementalRelayFee.GetFee(nSize)) {
                return state.DoS(
                    0, false, REJECT_INSUFFICIENTFEE, "insufficient fee", false,
                    strprintf(
                        "rejecting replacement %s, not enough additional fees "
                        "to relay; %s < %s",
        ...
        if (!CheckInputs(tx, state, view, true, scriptVerifyFlags, true,
            ...
            // need to turn both off, and compare against just turning off
            // CLEANSTACK
            // to see if the failure is specifically due to witness validation.
            CValidationState stateDummy; // Want reported failures to be from
                                         // first CheckInputs
            if (!tx.HasWitness() && CheckInputs(tx, stateDummy, view, true,
                ...
                !CheckInputs(tx, stateDummy, view, true,
                             ...
                             true, txdata)) {
                // Only the witness is missing, so the transaction itself may be
                // fine.
        ...
        BOOST_FOREACH (const CTxMemPool::txiter it, allConflicting) {
            LogPrint("mempool", "replacing tx %s with %s for %s BTC additional "
                                "fees, %d delta bytes\n",
...
bool ConnectBlock(const CBlock &block, CValidationState &state,
    ...
    if (!hashAssumeValid.IsNull()) {
        ...
        // externally verified to have a valid history.
        // A suitable default value is included with the software and updated
        // from time to time.  Because validity
        //  relative to a piece of software is an objective fact these defaults
        ...
        if (it != mapBlockIndex.end()) {
            if (it->second->GetAncestor(pindex->nHeight) == pindex &&
                ...
                // The equivalent time check discourages hashpower from
                // extorting the network via DOS attack
                //  into accepting an invalid block through telling users they
                //  must manually set assumevalid.
                //  Requiring a software change or burying the invalid block,
                //  regardless of the setting, makes
                //  it hard to hide the implication of the demand.  This also
                //  avoids having release candidates
                //  that are hardly doing any signature verification at all in
                //  testing without having to
                //  artificially set the default assumed verified block further
                //  back.
                // The test against nMinimumChainWork prevents the skipping when
    ...
    // See BIP30 and http://r6.ca/blog/20120206T005236Z.html for more
    // information.
    // This logic is not necessary for memory pool transactions, as
    // AcceptToMemoryPool
    // already refuses previously-known transaction ids entirely.
    // This rule was originally applied to all blocks with a timestamp after
    // March 15, 2012, 0:00 UTC.
    // Now that the whole chain is irreversibly beyond that time it is applied
    // to all blocks except the
    ...
    // coinbases and thus other than starting
    // with the 2 existing duplicate coinbase pairs, not possible to create
    // overwriting txs.  But by the
    // time BIP34 activated, in each of the existing pairs the duplicate
    // coinbase had overwritten the first
    ...
    for (unsigned int i = 0; i < block.vtx.size(); i++) {
        ...
        if (!tx.IsCoinBase()) {
            ...
            bool fCacheResults = fJustCheck; /* Don't cache results if we're
                                                actually connecting blocks
 ...
 * by copying
 * pblock) - if that is not intended, care must be taken to remove the last
 * entry in
...
static CBlockIndex *FindMostWorkChain() {
    do {
        ...
        // chain and the candidate are valid.
            ...
            if (fFailedChain || fMissingData) {
                ...
                while (pindexTest != pindexFailed) {
                    if (fFailedChain) {
                        ...
                        // mapBlocksUnlinked,
                        // so that if the block arrives in the future we can try
                        // adding
...
static bool ActivateBestChainStep(CValidationState &state,
    ...
    while (fContinue && nHeight != pindexMostWork->nHeight) {
        ...
        // best tip, as we likely only need
    ...
    do {
        ...
        bool fInitialDownload;
        {
            ...
            { // TODO: Tempoarily ensure that mempool removals are notified
              // before
                // connected transactions.  This shouldn't matter, but the
                // abandoned
                // state of transactions in our wallet is currently cleared when
                // we
                // receive another notification and there is a race condition
                // where
                // notification of a connected conflict might cause an outside
                // process
                // to abandon a transaction and then have it inadvertantly
                // cleared by
...
bool CheckBlock(const CBlock &block, CValidationState &state,
    ...
    if (fCheckMerkleRoot) {
        ...
        // sequences
        // of transactions in a block without affecting the merkle root of a
        // block,
...
bool ContextualCheckBlock(const CBlock &block, CValidationState &state,
    ...
    // * There must be at least one output whose scriptPubKey is a single
    // 36-byte push, the first 4 bytes of which are
    //   {0xaa, 0x21, 0xa9, 0xed}, and the following 32 bytes are
    //   SHA256^2(witness root, witness nonce). In case there are
    ...
    if (VersionBitsState(pindexPrev, consensusParams,
        ...
        if (commitpos != -1) {
            ...
            if (memcmp(hashWitness.begin(),
                ...
                return state.DoS(
                    100, false, REJECT_INVALID, "bad-witness-merkle-match",
                    true, strprintf("%s : witness merkle commitment mismatch",
                                    __func__));
            }
            fHaveWitness = true;
        }
    ...
    // as this would otherwise leave room for spam
    if (!fHaveWitness) {
...
static bool AcceptBlock(const std::shared_ptr<const CBlock> &pblock,
    ...
    // good...RELAY NOW
    // (but if it does not build on our best tip, let the SendMessages loop
    // relay it)
...
void static CheckBlockIndex(const Consensus::Params &consensusParams) {
    ...
    // before ActivateBestChain,
    // so we have the genesis block in mapBlockIndex but no active chain.  (A
    // few of the tests when
    ...
    while (pindex != NULL) {
        ...
        if (!CBlockIndexWorkComparator()(pindex, chainActive.Tip()) &&
            ...
            if (pindexFirstInvalid == NULL) {
                ...
                // setBlockIndexCandidates.  chainActive.Tip() must also be
                // there
                ...
                // missing data.
                // In this case it must be in mapBlocksUnlinked -- see test
                // below.
            }
        ...
        // This is a leaf node.
        // Move upwards until we reach a node of which we have not yet visited
        // the last child.
        while (pindex) {
...
bool LoadMempool(void) {
    ...
    try {
        ...
        while (num--) {
            ...
            if (amountdelta) {
                mempool.PrioritiseTransaction(tx->GetHash(),
                                              tx->GetHash().ToString(),
