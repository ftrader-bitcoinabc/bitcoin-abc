...
UniValue getbalance(const JSONRPCRequest &request) {
    ...
    if (request.fHelp || request.params.size() > 3)
        throw runtime_error(
            ...
            "                     the bumpfee command), temporarily resulting "
            "in low or even negative\n"
            "                     balances. In general, account balance "
            "calculation is not considered\n"
            "                     reliable and has resulted in confusing "
            "outcomes, so it is recommended to\n"
...
UniValue addwitnessaddress(const JSONRPCRequest &request) {
    ...
    if (request.fHelp || request.params.size() < 1 ||
        ...
        string msg = "addwitnessaddress \"address\"\n"
                     ...
                     "\nArguments:\n"
                     "1. \"address\"       (string, required) An address known "
                     "to the wallet\n"
    ...
    if (!ret) {
        throw JSONRPCError(RPC_WALLET_ERROR, "Public key or redeemscript not "
                                             "known to wallet, or the key is "
                                             "uncompressed");
    }
...
int64_t CalculateMaximumSignedTxSize(const CTransaction &tx) {
    ...
    for (auto &input : tx.vin) {
        ...
        assert(mi != pwalletMain->mapWallet.end() &&
...
UniValue bumpfee(const JSONRPCRequest &request) {
    ...
    if (request.fHelp || request.params.size() < 1 ||
        ...
        throw runtime_error(
            ...
            "a new transaction B.\n"
            "An opt-in RBF transaction with the given txid must be in the "
            "wallet.\n"
            "The command will pay the additional fee by decreasing (or perhaps "
            "removing) its change output.\n"
            "If the change output is not big enough to cover the increased "
            "fee, the command will currently fail\n"
            "instead of adding new inputs to compensate. (A future "
            "implementation could improve this.)\n"
            "The command will fail if the wallet or mempool contains a "
            "transaction that spends one of T's outputs.\n"
            "By default, the new fee will be calculated automatically using "
            "estimatefee.\n"
            "The user can specify a confirmation target for estimatefee.\n"
            "Alternatively, the user can specify totalFee, or use RPC "
            "setpaytxfee to set a higher fee rate.\n"
            "At a minimum, the new fee rate must be high enough to pay an "
            "additional new relay fee (incrementalfee\n"
            "returned by getnetworkinfo) to enter the node's mempool.\n"
            "\nArguments:\n"
            "1. txid                  (string, required) The txid to be "
            "bumped\n"
            "2. options               (object, optional)\n"
            "   {\n"
            "     \"confTarget\"        (numeric, optional) Confirmation "
            "target (in blocks)\n"
            "     \"totalFee\"          (numeric, optional) Total fee (NOT "
            "feerate) to pay, in satoshis.\n"
            "                         In rare cases, the actual fee paid might "
            "be slightly higher than the specified\n"
            "                         totalFee if the tx change output has to "
            "be removed because it is too close to\n"
            "                         the dust threshold.\n"
            "     \"replaceable\"       (boolean, optional, default true) "
            ...
            "                         still be replacable in practice, for "
            "example if it has unconfirmed ancestors which\n"
            "                         are replaceable).\n"
            "   }\n"
            "\nResult:\n"
            "{\n"
            "  \"txid\":    \"value\",   (string)  The id of the new "
            "transaction\n"
            "  \"origfee\":  n,         (numeric) Fee of the replaced "
            "transaction\n"
            "  \"fee\":      n,         (numeric) Fee of the new transaction\n"
            "  \"errors\":  [ str... ] (json array of strings) Errors "
            "encountered during processing (may be empty)\n"
            "}\n"
            "\nExamples:\n"
    ...
    RPCTypeCheck(request.params,
                 boost::assign::list_of(UniValue::VSTR)(UniValue::VOBJ));
    uint256 hash;
    hash.SetHex(request.params[0].get_str());
    ...
    if (wtx.GetDepthInMainChain() != 0) {
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY,
                           "Transaction has been mined, or is conflicted with "
                           "a mined transaction");
    }
    ...
    if (wtx.mapValue.count("replaced_by_txid")) {
        throw JSONRPCError(RPC_INVALID_REQUEST,
                           strprintf("Cannot bump transaction %s which was "
                                     "already bumped by transaction %s",
                                     hash.ToString(),
                                     wtx.mapValue.at("replaced_by_txid")));
    }

    // check that original tx consists entirely of our inputs
    // if not, we can't bump the fee, because the wallet has no way of knowing
    // the value of the other inputs (thus the fee)
    ...
    if (request.params.size() > 1) {
        ...
        if (options.exists("confTarget") && options.exists("totalFee")) {
            throw JSONRPCError(RPC_INVALID_PARAMETER,
                               "confTarget and totalFee options should not "
                               "both be set. Please provide either a "
                               "confirmation target for fee estimation or an "
            ...
            if (totalFee < requiredFee) {
                throw JSONRPCError(RPC_INVALID_PARAMETER,
                                   strprintf("Insufficient totalFee (cannot be "
                                             "less than required fee %s)",
    ...
    if (totalFee > 0) {
        ...
        if (totalFee < minTotalFee) {
            throw JSONRPCError(
                RPC_INVALID_PARAMETER,
                strprintf(
                    "Insufficient totalFee, must be at least %s (oldFee %s + "
                    "incrementalFee %s)",
        ...
        // if user specified a confirm target then don't consider any global
        // payTxFee
        if (specifiedConfirmTarget) {
            nNewFee = CWallet::GetMinimumFee(maxNewTxSize, newConfirmTarget,
                                             mempool, CAmount(0));
        }
        // otherwise use the regular wallet logic to select payTxFee or default
        // confirm target
        else {
            nNewFee =
                CWallet::GetMinimumFee(maxNewTxSize, newConfirmTarget, mempool);
        }

        nNewFeeRate = CFeeRate(nNewFee, maxNewTxSize);

        // New fee rate must be at least old rate + minimum incremental relay
        // rate
    ...
    if (nNewFee > maxTxFee) {
        throw JSONRPCError(RPC_MISC_ERROR,
                           strprintf("Specified or calculated fee %s is too "
                                     "high (cannot be higher than maxTxFee %s)",
                                     FormatMoney(nNewFee),
    ...
    // the mempool)
    // This may occur if the user set TotalFee or paytxfee too low, if
    // fallbackfee is too low, or, perhaps,
    ...
    if (nNewFeeRate.GetFeePerK() < minMempoolFeeRate.GetFeePerK()) {
        throw JSONRPCError(
            RPC_MISC_ERROR,
            strprintf("New fee rate (%s) is less than the minimum fee rate "
                      "(%s) to get into the mempool. totalFee value should to "
                      "be at least %s or settxfee value should be at least %s "
                      "to add transaction.",
    ...
    for (auto &input : tx.vin) {
        std::map<uint256, CWalletTx>::const_iterator mi =
            pwalletMain->mapWallet.find(input.prevout.hash);
        assert(mi != pwalletMain->mapWallet.end() &&
               input.prevout.n < mi->second.tx->vout.size());
        ...
        if (!ProduceSignature(TransactionSignatureCreator(pwalletMain,
                                                          &txNewConst, nIn,
                                                          amount, SIGHASH_ALL),
    ...
    if (!pwalletMain->CommitTransaction(wtxBumped, reservekey, g_connman.get(),
                                        state)) {
        // NOTE: CommitTransaction never returns false, so this should never
        // happen.
        throw JSONRPCError(
            RPC_WALLET_ERROR,
            strprintf("Error: The transaction was rejected! Reason given: %s",
    ...
    if (!pwalletMain->MarkReplaced(wtx.GetHash(), wtxBumped.GetHash())) {
        ...
        vErrors.push_back("Error: Created new bumpfee transaction but could "
                          "not mark the original transaction as replaced.");
    }
...
static const CRPCCommand commands[] = {
    ...
    {"wallet",
     ...
     &listunspent,
     false,
     {"minconf", "maxconf", "addresses", "include_unsafe"}},
