...
bool OldEncrypt(const CKeyingMaterial &vchPlaintext,
    ...
    if (fOk)
        fOk =
            EVP_EncryptInit_ex(ctx, EVP_aes_256_cbc(), NULL, chKey, chIV) != 0;
    if (fOk)
...
bool OldDecrypt(const std::vector<unsigned char> &vchCiphertext,
    ...
    if (fOk)
        fOk =
            EVP_DecryptInit_ex(ctx, EVP_aes_256_cbc(), NULL, chKey, chIV) != 0;
    if (fOk)
...
public:
    ...
    static void TestDecrypt(const CCrypter &crypt,
        ...
        // change
        // that would zero all padding except for the last byte for failed
        // decrypts.
