...
BOOST_AUTO_TEST_CASE(coin_selection_tests) {
    ...
    for (int i = 0; i < RUN_TESTS; i++) {
        ...
        BOOST_CHECK_EQUAL(setCoinsRet.size(), 3U); // the best should be
                                                   // 20+10+5.  it's incredibly
                                                   // unlikely the 1 or 2 got
        ...
        BOOST_CHECK(wallet.SelectCoinsMinConf(1 * MIN_CHANGE, 1, 1, 0, vCoins,
                                              setCoinsRet, nValueRet));
        BOOST_CHECK_EQUAL(nValueRet,
                          1111 * MIN_CHANGE); // we get the bigger coin
        BOOST_CHECK_EQUAL(setCoinsRet.size(), 1U);
        ...
        BOOST_CHECK(wallet.SelectCoinsMinConf(MIN_CHANGE, 1, 1, 0, vCoins,
                                              setCoinsRet, nValueRet));
        BOOST_CHECK_EQUAL(nValueRet,
                          MIN_CHANGE); // we should get the exact amount
        ...
        // test randomness
        {
            ...
            // but does depend on randomness in the stochastic approximation
            // code
            ...
            for (int j = 0; j < RANDOM_REPEATS; j++) {
                ...
                // this test will fail 1% of the time
                // run the test RANDOM_REPEATS times and only complain if all of
                // them fail
