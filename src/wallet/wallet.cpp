...

    {
        ...
        if (fFileBacked) {
            if (!pwalletdbEncryption->TxnCommit()) {
                ...
                // die to avoid confusion and let the user reload the
                // unencrypted wallet.
                assert(false);
            }
        ...
        // keep
        // bits of the unencrypted private key in slack space in the database
        // file.
            ...
            if (mi != pwallet->mapRequestCount.end()) {
                ...
                if (nRequests == 0 && !hashUnset()) {
                    ...
                    if (_mi != pwallet->mapRequestCount.end())
                        ...
                        nRequests = 1; // If it's in someone else's block it
                                       // must have got out
                }
            }
        }
...

    {
        ...
        for (map<uint256, CWalletTx>::const_iterator it = mapWallet.begin();
            ...
            if (nDepth == 0 && fOnlyConfirmed &&
                pcoin->mapValue.count("replaces_txid")) {
                continue;
            }
...
bool CWallet::SelectCoinsMinConf(
    ...
    BOOST_FOREACH (const COutput &output, vCoins) {
        ...
        if (!mempool.TransactionWithinChainLimit(pcoin->GetHash(),
                                                 nMaxAncestors))
            continue;
            ...
            while (true) {
                ...
                for (const auto &recipient : vecSend) {
                    ...
                    if (recipient.fSubtractFeeFromAmount) {
                        txout.nValue -=
                            nFeeRet / nSubtractFeeFromAmount; // Subtract fee
                                                              // equally from
                                                              // each selected
                                                              // recipient

                        if (fFirst) // first receiver pays the remainder not
                                    // divisible by output count
                        {
                            fFirst = false;
                ...
                for (const auto &pcoin : setCoins) {
                    ...
                    // reflecting an assumption the user would accept a bit more
                    // delay for
                ...
                if (nChange > 0) {
                    ...
                    // no coin control: send change to newly generated address
                        ...
                        // obvious which side is the change.
                        //  The drawback is that by not reusing a previous key,
                        //  the change may be lost if a
                        //  backup is restored, if the backup doesn't have the
                        //  new private key for the change.
                        //  If we reused the old key, it would be possible to
                        //  add code to look for and
                        //  rediscover unknown transactions that were written
                        //  with keys of ours to recover
                        //  post-backup change.
                    ...
                    // would end up paying more than requested.
                    // This would be against the purpose of the all-inclusive
                    // feature.
                    // So instead we raise the change and deduct from the
                    // recipient.
                    if (nSubtractFeeFromAmount > 0 &&
                        ...
                        CAmount nDust =
                            ...
                            newTxOut.nValue;
                        newTxOut.nValue +=
                            nDust; // raise change until no more dust
                        for (unsigned int i = 0; i < vecSend.size();
                             i++) // subtract from first recipient
                        {
                ...
                for (const auto &coin : setCoins)
                    txNew.vin.push_back(
                        CTxIn(coin.first->GetHash(), coin.second, CScript(),
...
bool CWallet::NewKeyPool() {
    {
        ...
        int64_t nKeys =
            max(GetArg("-keypool", DEFAULT_KEYPOOL_SIZE), (int64_t)0);
...
void CWallet::GetKeyBirthTimes(
    ...
    for (std::map<CKeyID, CBlockIndex *>::const_iterator it =
        ...
        mapKeyBirth[it->first] =
            it->second->GetBlockTime() - 7200; // block times can be 2h off
}
